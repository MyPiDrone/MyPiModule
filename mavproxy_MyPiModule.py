...
class MyPiModule(mp_module.MPModule):
    def __init__(self, mpstate):
        ...
        self.master2 = mavutil.mavlink_connection("udp:127.0.0.1:14550", input=False, source_system=self.settings.source_system, dialect="common")
    ...
    def my_statustext_send(self,text):
        ...
        self.master2.mav.statustext_send(6, " %02d %s" % (self.mycountermessage,text))
        self.say(text)
    ...
    def my_rc_check(self):
       if time.time() > self.last_rc_check_time + self.settings.mytimerc:
           ...
           if self.myrcraw[self.settings.myrcwlan] < self.RC_low_mark[self.settings.myrcwlan] and self.myrcraw[self.settings.myrcwlan] > (self.RC_low_mark[self.settings.myrcwlan]-100):
               ...
               msg = "MyRC%sRaw %s %s is up : %s : unknown RC value" % (self.settings.myrcwlan,self.myrcraw[self.settings.myrcwlan],self.settings.mywlan,self.wlan_up)
               self.my_write_log("WARNING",msg)
