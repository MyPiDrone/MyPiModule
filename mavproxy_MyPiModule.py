    ...
    def my_telemetry_text(self):
        if (time.time() > self.last_TText_check_time + self.settings.mytimeTText):
            ...
            if self.shutdown_by_lowbat == True or self.mystate == 5 or self.mystate == 6:
                ...
                color='black'
                level='_'
            intext = "%s %s %s %s %s %s %s %s %s %s %s Thr=%s %s %s GPSSpeed=%s %sV Cur=%s Remain=%s'%' ALt=%sm" % (mytime,level,["Disarmed","Armed   "][self.armed == True],self.mystatename[self.mystate],self.status.flightmode,self.myTText_gps,self.myTText_heading,["NetDown","NetUP  "][self.net_up == True],["VideoOFF","VideoON "][self.video_on == True],["___","RTL"][self.rtl_on == True],["_________","STABILIZE"][self.stabilize_on == True],self.mythrottle,self.myTText_Roll,self.myTText_Pitch,self.mygroundspeed,str(round(self.myvolt/1000,2)),str(round(self.mycurrenti/100,1),self.myremaining,self.status.altitude)
            # max 255
