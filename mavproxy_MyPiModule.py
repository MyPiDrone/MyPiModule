...
class MyPiModule(mp_module.MPModule):
    def __init__(self, mpstate):
        ...
        self.outpipe = open(self.settings.mypipeout, 'a')
...
#        self.camera.framerate = 30
    ...
    def my_telemetry_text(self):
        ...
        if (time.time() > self.last_TText_check_time + self.settings.mytimeTText):
            ...
            if flying and not self.in_air:
                ...
                myTText_FlightTime="FT=%u:%02u/%u:%02u" % (int(self.total_time)/60, int(self.total_time)%60,int(current_all_total_time)/60, int(current_all_total_time)%60)
            elif not flying and self.in_air:
                self.in_air = False
                self.camera.stop_recording(splitter_port=2)
                self.total_time = time.mktime(self.timestamp) - self.start_time
                self.all_total_time = self.all_total_time + self.total_time
                myTText_FlightTime="FT=%u:%02u/%u:%02u" % (int(self.total_time)/60, int(self.total_time)%60,int(self.all_total_time)/60, int(self.all_total_time)%60)
            else:
                myTText_FlightTime="FT=%u:%02u/%u:%02u" % (int(self.total_time)/60, int(self.total_time)%60,int(self.all_total_time)/60, int(self.all_total_time)%60)
            ##################################################################################
            # re-used code mavproxy_console.py
            ...
            # myTText_Radio
            ##################################################################################
            myTText_Radio="RSSI=%s" % (str(self.my_rssi))
            ##############################
            ...
            myTText="{0}\nL{1}c{2}%w{3}%".format(myTText,math.ceil(os.getloadavg()[0]*10)/10,psutil.cpu_times_percent()[0],psutil.cpu_times_percent()[4])
            myTText="{0} {1}".format(myTText,myTText_Radio)
            myTText="{0} {1}".format(myTText,myTText_GPS)
            myTText="{0} GSpd={1}".format(myTText,math.ceil(self.mygroundspeed*10)/10)
            myTText="{0} SM{1:3}".format(myTText,["OFF","ON"][self.simple_mode_on == True])
            ...
            if mysnapshottime != self.snapshottime and (self.in_air == True or (self.mystate != 3 and self.mystate != 4) or self.shutdown_by_lowbat == True or self.reboot_by_radio == True or self.shutdown_by_radio == True):
                ...
                self.camera.capture(jpgname, format='jpeg', use_video_port=True, splitter_port=0)
    ...
    def my_network_status(self):
            ...
            if rc == 0:
                ...
                self.net_up = True
                print("net rc=%s %s" % (rc,self.net_ip_current))
            else:
                self.net_ip_current = "null" 
                self.net_up = False 
    ...
    def my_video_recording_status(self):
            ...
            if os.path.exists(self.h264name):
                ...
                if currentsize.st_size != self.video_recording_size:
                    print("Video recording in progress : Current size:{0}Meg".format(currentsize.st_size/1048576))
                    self.video_recording_size = currentsize.st_size
                    self.video_recording_on = True
                else:
                    print("Video recording paused : Current size:{0}Meg".format(currentsize.st_size/1048576))
                    self.video_recording_on = False
            else:
                self.video_recording_on = False
    ...
    def my_statustext_send(self,text):
        ...
        print("self.settings.source_system=%s text=%s net_up=%s" % (self.settings.source_system,text,self.net_up))
       ...
       if time.time() > self.last_rc_check_time + self.settings.mytimerc:
           ...
           if self.myrcraw[self.settings.myrcnet] < self.RC_low_mark[self.settings.myrcnet] and self.myrcraw[self.settings.myrcnet] > (self.RC_low_mark[self.settings.myrcnet]-150):
               ...
               if self.net_up == False:
                   ...
                   if self.net_up_prev != self.net_up: self.my_statustext_send("%s upX %s %s" % (self.settings.myinterfaceadmin,self.net_ip_current,self.net_up_prev))
                   self.net_up_prev = self.net_up
