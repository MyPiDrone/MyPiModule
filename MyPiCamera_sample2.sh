...
##############################################################################################################################
####### www.MyPiDrone.com
#TITLE# DRONE MyPiCamera start MyPiCamera_sample2.py & tx (python sample with picamera)
####### www.MyPiDrone.com
##############################################################################################################################
###############################################################################
# V1 camera
...
if [ ! -p /tmp/MyPiCamera.pipeout ]; then
	mkfifo /tmp/MyPiCamera.pipeout
fi
echo "$WifiBroadcast_TX -p $PORT -b $BLOCK_SIZE -r $FECS -f $PACKET_LENGTH $WLAN 2>&1 < /tmp/MyPiCamera.pipeout"
$WifiBroadcast_TX -p $PORT -b $BLOCK_SIZE -r $FECS -f $PACKET_LENGTH $WLAN 2>&1 < /tmp/MyPiCamera.pipeout
PID=$!
sleep 3
#echo 'My telemetry text' > /tmp/MyPiCamera.pipein
./MyPiCamera_sample.py 
sleep 20
kill $PID
