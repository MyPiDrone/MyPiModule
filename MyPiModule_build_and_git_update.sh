...
git config --global user.email "MyPiDrone@gmail.com"
git config --global user.name "MyPiDrone"
###########################################################
date=`date +'%Y-%m-%d'`
...
#
#OBSOLETE#cp /lib/systemd/system/ArduCopter-quad.service                       ${MY_DIR_MYPIMODULE}
cp /lib/systemd/system/mavproxy.service                              ${MY_DIR_MYPIMODULE}
#OBSOLETE#cp /etc/rc.local                                                     ${MY_DIR_MYPIMODULE}
#
cp /var/APM/ArduCopter.stg                                           ${MY_DIR_MYPIMODULE}
#
cp /usr/local/bin/start_MAVProxy_MyPiModule.sh                       ${MY_DIR_MYPIMODULE}
#OBSOLETE#cp /usr/local/bin/start_ArduCopter-quad.sh                           ${MY_DIR_MYPIMODULE}
#NOT_USED#cp /usr/local/bin/start_tx_and_recording_with_picamera_video_input.sh                ${MY_DIR_MYPIMODULE}
#NOT_USED#cp /usr/local/bin/start_tx_and_recording_with_raspivid_video_input_on_wifiap.sh      ${MY_DIR_MYPIMODULE}
#NOT_USED#cp /usr/local/bin/start_tx_and_recording_with_raspivid_video_input.sh                ${MY_DIR_MYPIMODULE}
cp /usr/local/bin/manage_network.sh                                  ${MY_DIR_MYPIMODULE}
cp /usr/local/bin/show_modules.sh                                    ${MY_DIR_MYPIMODULE}
#NOT_USED#cp /usr/local/bin/start_wlan1_mode_monitor.sh                        ${MY_DIR_MYPIMODULE}
#NOT_USED#cp /usr/local/bin/start_wlan1_mode_managed.sh                        ${MY_DIR_MYPIMODULE}
#
...
#echo $MSG
echo "/usr/bin/python $MAVPROXY --master=udp:127.0.0.1:14550 --quadcopter --out=/dev/ttyUSB0,57600  --default-modules='MyPiModule,mode' --show-errors --moddebug=3"
### load only MyPiModule and mode
/usr/bin/python $MAVPROXY --master=udp:127.0.0.1:14550 --quadcopter --out=/dev/ttyUSB0,57600  --default-modules='MyPiModule,mode' --show-errors --moddebug=3
### load all modules
