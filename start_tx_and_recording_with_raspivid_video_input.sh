...
then
        ...
        if [ "$OPTION" = "--vrt" -o "_$OPTION" = "_" ]; then
                ...
                if [ "$OPTION" = "--vr" ]; then
                        ...
                        echo "raspivid -ih -t $TIMEOUT -w $WIDTH -h $HEIGHT -fps $FPS -b $BITRATE -n -g $KEYFRAMERATE -pf high -o - | $WifiBroadcast_TX -p $PORT -b $BLOCK_SIZE -r $FECS -f $PACKET_LENGTH $WLAN" 
